...
float4 LightingMainPixelShader(LightingMainVertexOutput input) : SV_Target
{
	...
	lightingOutput += CalculateSpecularIBL(pfSpecularIBLMipLevels, lerp(float3(0.0f, 0.0f, 0.0f), surface.albedo.rgb, metallic), roughness, nDotV, reflection, EnvironmentMap, EnvBRDFLookupTexture, EnvironmentSampler); 
	
	if(nDotL > 0.0f)
	{
		float3 half = normalize(viewDir + pfLightDir.xyz);
        float nDotHalf = saturate(dot(surface.normal, half));
		float3 surfaceSpec = lerp(float3(1.0, 1.0, 1.0), surface.albedo.rgb, metallic);
		float  specMetallic = max(metallic, 0.03);
		
		float3 specularFactor = CalculateFresnelShlick(float3(specMetallic, specMetallic, specMetallic) * surfaceSpec, viewDir, half) *
			  CalculateGeometryTermSmithGGX(roughness, nDotL, nDotV) *
			  CalculateNormalDistributionTW(roughness, nDotHalf) *
		...
		lightingOutput.rgb += (diffuseFactor + specularFactor) * pfLightIntensity * pfLightColor * shadowMultiplier;
	}
