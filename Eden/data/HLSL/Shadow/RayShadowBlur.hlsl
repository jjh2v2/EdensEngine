...
float RayShadowBlurPixelShader(RayShadowBlurVertexOutput input) : SV_Target
{
    ...
    for (uint y = 0; y < 32; y++)
    {
        softShadowResult += ShadowInput.Sample(ShadowSamplerLinear, input.texCoord0 + PoissonSamples[y] * oneOverscreenSize * 6) > 0.0001 ? 1 : 0;
    }
    
    hardShadowResult /= 16;
    softShadowResult /= 32;
    
    float shadowResult = lerp(hardShadowResult, softShadowResult, shadowTransitionFactor);
