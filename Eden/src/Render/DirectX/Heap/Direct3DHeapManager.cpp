...
Direct3DHeapManager::Direct3DHeapManager(ID3D12Device* device)
{
	...
	mSRVDescriptorHeap = new DescriptorHeap(device, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, SRV_DESCRIPTOR_HEAP_SIZE, true);
	mDSVDescriptorHeap = new DescriptorHeap(device, D3D12_DESCRIPTOR_HEAP_TYPE_DSV, DSV_DESCRIPTOR_HEAP_SIZE, false);
	mSamplerDescriptorHeap = new DescriptorHeap(device, D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, SAMPLER_DESCRIPTOR_HEAP_SIZE, true);
}
