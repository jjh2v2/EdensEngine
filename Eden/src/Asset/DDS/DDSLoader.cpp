...
size_t DDSLoader::GetBitsPerPixel(DXGI_FORMAT format)
{
	switch (format)
	{
...
void DDSLoader::GetSurfaceInfo(size_t width, size_t height, DXGI_FORMAT format, size_t* outNumBytes, size_t* outRowBytes, size_t* outNumRows)
{
	size_t numBytes = 0;
	size_t rowBytes = 0;
	size_t numRows = 0;
	...
	if (bc)
	{
		...
		if (width > 0)
		{
			numBlocksWide = MathHelper::MaxSize(1, (width + 3) / 4);
		}

		size_t numBlocksHigh = 0;
		if (height > 0)
		{
			numBlocksHigh = MathHelper::MaxSize(1, (height + 3) / 4);
		}
	...
	{
		size_t bpp = GetBitsPerPixel(format);
...
bool DDSLoader::IsBitMask(DirectX::DDS_PIXELFORMAT pixelFormat, uint32 r, uint32 g, uint32 b, uint32 a)
{
	return (pixelFormat.RBitMask == r && pixelFormat.GBitMask == g && pixelFormat.BBitMask == b && pixelFormat.ABitMask == a);
}


DXGI_FORMAT DDSLoader::GetDXGIFormat(const DirectX::DDS_PIXELFORMAT pixelFormat)
{
...
DXGI_FORMAT DDSLoader::MakeSRGB(DXGI_FORMAT format)
{
	switch (format)
	{
	case DXGI_FORMAT_R8G8B8A8_UNORM:
...
HRESULT DDSLoader::FillInitData(size_t width, size_t height, size_t depth, size_t mipCount, size_t arraySize,
...
HRESULT DDSLoader::CreateD3DResources(_In_ ID3D12Device* d3dDevice,
...
HRESULT DDSLoader::CreateTextureFromDDS(_In_ ID3D12Device* d3dDevice, _In_ const DirectX::DDS_HEADER* header, _In_reads_bytes_(bitSize) const uint8_t* bitData,
